...
const Block &BlockMap::GetBlock(int r, int c) const
{
  ASSERT(ValidCoord(Vector2(r, c)));
  return m_BlockMatrix[r][c];
}

const Block &BlockMap::GetBlock(const Vector2 &vec) const
{
  ASSERT(ValidCoord(vec));
  return m_BlockMatrix[vec.r][vec.c];
}
...
void BlockMap::ResetConnectivity()
{
  for (int r = 0; r < Rows(); r++)
  {
    for (int c = 0; c < Cols(); c++)
    {
      ...
      Vector2 tOffset(-1, 0);
      Vector2 bOffset(1, 0);
      if (ValidCoord(currBlock.coord + lOffset))
        currBlock.neighborOffsets.insert(lOffset);
      if (ValidCoord(currBlock.coord + rOffset))
        currBlock.neighborOffsets.insert(rOffset);
      if (ValidCoord(currBlock.coord + tOffset))
        currBlock.neighborOffsets.insert(tOffset);
      if (ValidCoord(currBlock.coord + bOffset))
        currBlock.neighborOffsets.insert(bOffset);
    }
  }
}
